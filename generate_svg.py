...
def make_svg(stats):
    ...
    ASCII_FONT   = 6.5      # más pequeño para que el ASCII grande quepa
    ASCII_LINE_H = 8.8
    INFO_FONT    = 12.5
    INFO_LINE_H  = 20.0
    ...
    W           = INFO_X + 560
    # Alto = el mayor de los dos bloques + padding generoso
    H           = int(max(ASCII_H, info_h) + PAD * 5)

    MONO        = "JetBrains Mono, Fira Code, Consolas, monospace"
    ASCII_COLOR = "#5bafd6"   # azul claro visible sobre fondo oscuro
...
if __name__ == "__main__":
    ...
    with open("profile.svg", "w", encoding="utf-8") as f:
        f.write(svg)
    print(f"Listo: profile.svg ({len(svg):,} bytes, alto={svg.split('height=')[1].split('\"')[1]}px)")
